...
#ifdef OPENSBI_PLATFORM_ESP32S31_CLIC_TIMER_IRQ
static int esp32s31_clic_timer_irq_redirect(struct sbi_trap_regs *regs)
{
	int rc;
	unsigned long scause;
	struct sbi_trap_info stimer_trap = {
		.cause = MCAUSE_IRQ_MASK | IRQ_S_TIMER,
		.tval = 0,
		.tval2 = 0,
		.tinst = 0,
		.gva = 0,
	...
	rc = sbi_trap_redirect(regs, &stimer_trap);
	scause = MCAUSE_IRQ_MASK | IRQ_S_TIMER;
	if (csr_read(CSR_SSTATUS) & SSTATUS_SPP)
		scause |= (1UL << 28);
	if (csr_read(CSR_SSTATUS) & SSTATUS_SPIE)
		scause |= (1UL << 27);
	csr_write(CSR_SCAUSE, scause);
	return rc;
}
#endif
...
#ifdef OPENSBI_PLATFORM_ESP32S31_CLIC_TIMER_IRQ
		if ((mcause & ~MCAUSE_IRQ_MASK) ==
		    OPENSBI_PLATFORM_ESP32S31_CLIC_TIMER_IRQ) {
			rc = esp32s31_clic_timer_irq_redirect(regs);
			msg = "ESP32-S31 CLIC timer redirect failed";
			goto trap_done;
		}
#endif
		/*
		 * ESP32-S31 reports MIDELEG interrupt bits as read-only zero.
		 * When OpenSBI injects an S-mode timer interrupt by setting STIP,
		 * the trap therefore re-enters M-mode as IRQ_S_TIMER. Redirect it
		 * to S-mode explicitly instead of treating it as an unhandled local
		 * interrupt. This is intentionally platform-gated because normal
		 * RISC-V platforms rely on writable mideleg.STIP.
		 */
		if ((mcause & ~MCAUSE_IRQ_MASK) == IRQ_S_TIMER) {
			csr_clear(CSR_MIP, MIP_STIP);
			rc = sbi_trap_redirect(regs, trap);
			msg = "S-mode timer redirect failed";
			goto trap_done;
		}
#endif
